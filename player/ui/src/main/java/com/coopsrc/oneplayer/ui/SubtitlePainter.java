...
/* package */ final class SubtitlePainter {
  ...
  public void draw(
    ...
    if (areCharSequencesEqual(this.cueText, cue.text)
        && PlayerUtils.areEqual(this.cueTextAlignment, cue.textAlignment)
        && this.cueBitmap == cue.bitmap
        && this.cueLine == cue.line
        && this.cueLineType == cue.lineType
        && PlayerUtils.areEqual(this.cueLineAnchor, cue.lineAnchor)
        && this.cuePosition == cue.position
        && PlayerUtils.areEqual(this.cuePositionAnchor, cue.positionAnchor)
        ...
        && PlayerUtils.areEqual(this.textPaint.getTypeface(), style.typeface)
  ...
  private void setupTextLayout() {
    ...
    if (availableWidth <= 0) {
      PlayerLogger.w(TAG, "Skipped drawing subtitle cue (insufficient space)");
      return;
    }
    ...
    if (textWidth <= 0) {
      PlayerLogger.w(TAG, "Skipped drawing subtitle cue (invalid horizontal positioning)");
      return;
    }
